d2BC_S_I_HC_raw_NNHM_JAGS<-function (df, hh, rlmc = 0.5, mu_mean = 0, mu_sd = 4,
                                     nchains=4, nadapt=4000, nburnin=20000,
                                     niter=120000, nthin=4){
  
  # Computation of S-I based on d2BC for NNHM with a HC heterogeneity prior in JAGS
  ...
  fit_P_0 <- jags_fit_NNHM_HC(df=df,
  ...
  fit_P_u <- jags_fit_NNHM_HC(df=df,
                              mu_prior_mean=mu_mean,
                              mu_prior_prec=1/(mu_sd^2),
  ...
  fit_P_l <- jags_fit_NNHM_HC(df=df,
                              mu_prior_mean=mu_mean,
                              mu_prior_prec=1/(mu_sd^2),
  ...
  fit_L_l <- jags_fit_NNHM_HC(df=dfps_l,
                              mu_prior_mean=mu_mean,
                              mu_prior_prec=1/(mu_sd^2),
                              tau_prior_prec=1/(tau_rlmc_0^2),
                              params=c("mu", "theta", "tau", "log_tau", "theta_new"),
  ...
  fit_L_u <- jags_fit_NNHM_HC(df=dfps_u,
                              mu_prior_mean=mu_mean,
                              mu_prior_prec=1/(mu_sd^2),
                              tau_prior_prec=1/(tau_rlmc_0^2),
                              params=c("mu", "theta", "tau", "log_tau", "theta_new"),
